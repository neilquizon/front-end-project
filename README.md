...
#### Mac Terminal, Windows Node.js CMD Prompt or PowerShell

```shell
$ npm install gulp -g 
```

**Note:** If you are working on a Mac and you receive a permission error after entering the above command in the terminal, follow the instructions on this page: [fix npm permission errors on macOS](https://github.com/htpwebdesign/fix-npm-permission-errors-on-mac-os) to fix the issue.
...
That's it! You have now installed this repo and are now ready to run and build this project.
...
Make sure your terminal application is pointed at the root directory of this repository.
...
### Q. Do I have to use jQuery?

In the "index.html" file located in the "dev" folder you will notice that the file includes a script tag that links to a local copy of the jQuery library located in the "dev/scripts/libs/jquery/" directory. 

If you would like to use jQuery then you do not have to do anything, just code your site and write your jQuery code in your script file. The gulpfile will handle copying the jQuery library to the "dist" folder when your run the "build" command.

If you do **NOT** want to use jQuery, then remove the jQuery script link from the "index.html" file and change the value of the "useJQuery" variable on line 39 of the "gulpfile.js" file to false. See below.
